...
import com.example.blindfoldedcube.CubeDataStructure.Cubies.Corner;
import com.example.blindfoldedcube.CubeDataStructure.Cubies.Edge;
import static com.example.blindfoldedcube.CubeDataStructure.Cubies.Corner.*;
import static com.example.blindfoldedcube.CubeDataStructure.Cubies.Edge.*;
//Based on Kociemba's work
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
public class CubieCube {
	...
	// corner permutation
	Corner[] cp = {UFR, UFL, UBL, UBR, DFR, DFL, DBL, DBR};
	...
	private static Corner[] cpU = { UBR, UFR, UFL, UBL, DFR, DFL, DBL, DBR};
	...
	private static Corner[] cpR = { DFR, UFL, UBL, UFR, DBR, DFL, DBL, UBR };
	...
	private static Corner[] cpF = { UFL, DFL, UBL, UBR, UFR, DFR, DBL, DBR};
	...
	private static Corner[] cpD = {UFR, UFL, UBL, UBR, DFL, DBL, DBR, DFR };
	...
	private static Corner[] cpL = {UFR, UBL, DBL, UBR, DFR, UFL, DFL, DBR};
	...
	private static Corner[] cpB = {UFR, UFL, UBR, DBR, DFR, DFL, UBL, DBL };
	...
	static {
		moveCube[0] = new CubieCube(); //TODO: modify to apply move on an existing cube
		moveCube[0].cp = cpU;
		moveCube[0].co = coU;
		moveCube[0].ep = epU;
		moveCube[0].eo = eoU;
	...
	short getTwist() {
		short ret = 0;
		for (int i = UFR.ordinal(); i < DBR.ordinal(); i++)
	...
	void setTwist(short twist) {
		int twistParity = 0;
		for (int i = DBR.ordinal() - 1; i >= UFR.ordinal(); i--) {
			twistParity += co[i] = (byte) (twist % 3);
			twist /= 3;
		}
		co[DBR.ordinal()] = (byte) ((3 - twistParity % 3) % 3);
	}
	...
	short cornerParity() {
		int s = 0;
		for (int i = DBR.ordinal(); i >= UFR.ordinal() + 1; i--)
			for (int j = i - 1; j >= UFR.ordinal(); j--)
	...
	short getURFtoDLF() {
		...
		for (int j = UFR.ordinal(); j <= DBR.ordinal(); j++)
			if (cp[j].ordinal() <= DFL.ordinal()) {
				a += Cnk(j, x + 1);
				corner6[x++] = cp[j];
			}
	...
	void setURFtoDLF(short idx) {
		int x;
		Corner[] corner6 = {UFR, UFL, UBL, UBR, DFR, DFL};
		Corner[] otherCorner = { DBL, DBR};
		int b = idx % 720; // Permutation
		int a = idx / 720; // Combination
		for (Corner c : Corner.values())
			cp[c.ordinal()] = DBR;// Use DRB to invalidate all corners
	...
	void setURFtoDLB(int idx) {
		Corner[] perm = {UFR, UFL, UBL, UBR, DFR, DFL, DBL, DBR};
		int k;
		for (int j = 1; j < 8; j++) {
			k = idx % (j + 1);
			idx /= j + 1;
	...
	public CubieCube UTurn (CubieCube aCube)
	{

		return new CubieCube(cpU, coU, epU, eoU);
	}
//	// this CubieCube array represents the 6 basic cube moves
//	public CubieCube[] moveCurrCube = new CubieCube[6];
//
//	{
//		moveCube[0] = new CubieCube(); //TODO: modify to apply move on an existing cube
//		moveCube[0].cp = cpU;
